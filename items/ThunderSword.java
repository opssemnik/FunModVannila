...
public class ThunderSword extends ItemSword {
	...
	public ThunderSword(int i, ToolMaterial toolmaterial) {
		super(toolmaterial);
	}
	...
	public void onUpdate(ItemStack par1ItemStack, World par2World,
		...
		if (par3Entity instanceof EntityPlayer) {
			...
			if (player.getCurrentEquippedItem().getItem() != null
					&& player.getCurrentEquippedItem().getItem() instanceof ThunderSword) {
				par2World.spawnParticle("smoke", player.posX, player.posY,
						player.posZ, 1.0D, 1.0D, 1.0D);
				par2World.spawnParticle("flame", player.posX, player.posY,
						player.posZ, 1.0D, 1.0D, 1.0D);
