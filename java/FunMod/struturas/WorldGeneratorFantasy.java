...
public class WorldGeneratorFantasy implements IWorldGenerator {
	public void generate(Random random, int chunkX, int chunkZ, World world,
		...
		case 10:
			generateFantasy(world, random, chunkX * 16, chunkZ * 16);

		}
	}

	private void generateFantasy(World world, Random random, int blockX,
			int blockZ) {
		int Xcoord = blockX + random.nextInt(16);
		int Ycoord = random.nextInt(60);
		int Zcoord = blockZ + random.nextInt(16);
		(new WorldGenMinableFantasy(Blocks.coal_ore, 19)).generate(world,
		...
		(new WorldGenMinableFantasy(Blocks.iron_ore, 10)).generate(world,
				random, Xcoord2, Ycoord2, Zcoord2);
		int Xcoord3 = blockX + random.nextInt(16);
		int Ycoord3 = random.nextInt(40);
		int Zcoord3 = blockZ + random.nextInt(16);
		(new WorldGenMinableFantasy(Blocks.gold_ore, 5)).generate(world,
				random, Xcoord3, Ycoord3, Zcoord3);
		int Xcoord4 = blockX + random.nextInt(16);
		int Ycoord4 = random.nextInt(14);
		int Zcoord4 = blockZ + random.nextInt(16);
		(new WorldGenMinableFantasy(Blocks.diamond_ore, 3)).generate(world,
				random, Xcoord4, Ycoord4, Zcoord4);
	}
	...
	private void generateSurface(World world, Random rand, int x, int z) {
		...
		if (rand.nextInt(1000) <= 100) {
			for (int i = 0; i < 15; ++i) {
				...
				if (new WorldGenCasaDoPokemon().generate(world, rand, RandPosX,
						RandPosY, RandPosZ)) {
					{
						break;
					}
		...
		if (rand.nextInt(1000) <= 100) {
			for (int i = 0; i < 15; ++i) {
				...
				if (new WorldGenLabirinto().generate(world, rand, RandPosX2,
						RandPosY2, RandPosZ2)) {
					{
						break;
					}
		...
		if (rand.nextInt(1000) <= 100) {
			for (int i = 0; i < 15; ++i) {
				...
				int RandPosY2 = rand.nextInt(150);
				int RandPosZ2 = z + rand.nextInt(16);
				if (new WorldGenBowserCastle().generate(world, rand, RandPosX2,
						RandPosY2, RandPosZ2)) {
					{
						break;
					}
